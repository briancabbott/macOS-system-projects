...
extension JSON.Value {
     ...
     */
    public func toString(options: JSON.Serializer.Option = []) throws -> String {
        return try JSON.Serializer.serialize(self, options: options)
    }
    
    /**
     Turns a nested graph of `JSON`s into `Data`. This produces JSON data that
     strictly conforms to [RFT7159](https://tools.ietf.org/html/rfc7159).
     It can optionally pretty-print the output for debugging, but this comes with a non-negligible performance cost.
     */
    public func toData(options: JSON.Serializer.Option = []) throws -> Data {
        let jsonString = try JSON.Serializer.serialize(self, options: options)
        return jsonString.toUTF8Data()
    }
}
...
public extension JSON {
  public struct Serializer {
    ...
    init(json: JSON.Value, options: Option = []) {
...
extension JSON.Serializer {
  public static func serialize<O: TextOutputStream>(_ json: JSON.Value, to stream: inout O, options: Option) throws {
    let writer = JSON.Serializer(json: json, options: options)
    try writer.writeValue(json, to: &stream)
  }

  public static func serialize(_ json: JSON.Value, options: Option = []) throws -> String {
    var s = ""
    let writer = JSON.Serializer(json: json, options: options)
...
extension JSON.Serializer {
  func writeValue<O: TextOutputStream>(_ value: JSON.Value, to stream: inout O, indentLevel: Int = 0) throws {
    switch value {
    case .array(let a):
...
extension JSON.Serializer {
  ...
  func writeArray<O: TextOutputStream>(_ a: JSON.Array, to stream: inout O, indentLevel: Int = 0) throws {
    if a.isEmpty {
      stream.write("[]")
      return
    }
  ...
  func writeObject<O: TextOutputStream>(_ o: JSON.Object, to stream: inout O, indentLevel: Int = 0) throws {
    if o.isEmpty {
      stream.write("{}")
      return
    }
